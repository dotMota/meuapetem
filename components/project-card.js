class ProjectCard extends HTMLElement {
    ...
    connectedCallback() {
        ...
        const vibe = this.getAttribute('vibe') || 'Lifestyle Urbano'; // Ex: "Para Home Office", "Luz da Manhã"
        const image = this.getAttribute('image');
        const link = this.getAttribute('link') || '#';
        const price = this.getAttribute('price') || 'Consulte';

        // Baseado na "Curadoria Experiencial" do PDF
        const tags = (this.getAttribute('tags') || '').split(',');

        this.shadowRoot.innerHTML = `
        <style>
            /* ... (Mesmo CSS base do anterior, com adições abaixo) ... */
            ...
            .content {
                position: absolute; bottom: 0; left: 0; width: 100%; padding: 2rem; box-sizing: border-box;
                display: flex; flex-direction: column; gap: 10px;
            }

            .vibe-tag {
                color: var(--highlight-color); font-size: 0.8rem; letter-spacing: 2px; text-transform: uppercase; font-weight: 700;
            }

            h3 { margin: 0; color: #fff; font-family: var(--font-title); font-size: 2rem; }

            .tags-container { display: flex; gap: 8px; flex-wrap: wrap; margin-top: 5px; }
            .tag {
                background: rgba(255,255,255,0.1); backdrop-filter: blur(5px);
                padding: 5px 12px; border-radius: 20px; font-size: 0.75rem; color: #ddd; border: 1px solid rgba(255,255,255,0.2);
            }

            .cta-btn {
                margin-top: 15px; padding: 15px; background: #fff; color: #000; text-align: center;
                border-radius: 8px; font-weight: 700; text-decoration: none; text-transform: uppercase;
